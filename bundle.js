...
var _canvas2svg = __webpack_require__(1);

var _canvas2svg2 = _interopRequireDefault(_canvas2svg);

var _dat = __webpack_require__(2);
...
var drawHarmonograph = function drawHarmonograph() {
	ctx.clearRect(0, 0, 800, 600);
	ctx.save();
	ctx.translate(400, 300);
	ctx.beginPath();
	for (var t = 0; t < 100; t += 0.01) {
		var x = first.amplitude * Math.sin(first.frequency * t + Math.PI * first.phase) * Math.exp(-first.delta * t) + second.amplitude * Math.sin(second.frequency * t + Math.PI * second.phase) * Math.exp(-second.delta * t);
		var y = third.amplitude * Math.sin(third.frequency * t + Math.PI * third.phase) * Math.exp(-third.delta * t) + fourth.amplitude * Math.sin(fourth.frequency * t + Math.PI * fourth.phase) * Math.exp(-fourth.delta * t);
		ctx.lineTo(x, y);
	}
	ctx.stroke();
	ctx.restore();
